	    ...
	    <div class="box-group" id="accordion">
		  ...
		  <div class="panel box box-primary">
			  ...
			  <div class="box-header with-border">
				...
				<a href="addPermissionToRole?id=${ r.id }&identity=${ r.identity }" class="btn btn-primary pull-right">添加</a>
			  </div>
			  <!-- 内容 -->
			  <div id="collapse${st.index}" class="panel-collapse collapse" aria-expanded="true">
				<div class="box-body">
				  ...
				  <table class="table table-hover table-striped">
					  ...
					  <c:forEach items="${r.permissions}" var="p" varStatus="st">
						<tr>
						  <td>${ st.index+1 }</td>
						  <td>${ p.name }</td>
						  <td>${ p.resource }</td>
						  <td>
						    <a href="deleteRolePermission?role_id=${ r.id }&permission_id=${ p.id }" class="btn btn-danger">
